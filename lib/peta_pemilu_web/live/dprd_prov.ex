defmodule PetaPemiluWeb.Live.DprdProv do
  ...
  def render(assigns) do
    ...
    <div class="pb-8">
      <div
        style={"background-image: url(#{static_path(@socket, "/images/wave-dprd-prov.svg")})"}
        class="bg-left-bottom bg-[length:auto_2rem] bg-repeat-x pb-8 mb-12 text-center"
      >
        <h1
          class="bg-[#1c5192] pt-6 px-8 text-white uppercase font-bold text-xl"
              ...
              <ol class="[counter-reset:listCounter]">
                <%= for c <- party.caleg do %>
                  <li class="[&:first-child]:border-t border-b border-gray-500 border-spacing-0 text-sm [counter-increment:listCounter] relative">
                    <%= if c["id"] do %>
                      <a
                        href={~p"/caleg/dprd-prov/#{assigns.dapil_slug}/#{c["id"]}"}
                        target="_blank"
